    ...
    public function authenticate(
        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                // Send encoded email and password
                if (!$this->sendCommand(
                    'User & Password',
                    base64_encode("\0" . $email . "\0" . $password),
                    235
                )
                ) {
                    return false;
            ...
            case 'LOGIN':
                ...
                if (!$this->sendCommand("email", base64_encode($email), 334)) {
                    return false;
                }
            ...
            case 'NTLM':
                ...
                // send encoded email
                return $this->sendCommand('email', base64_encode($msg3), 235);
                break;
            case 'CRAM-MD5':
                // Start authentication
                ...
                // Build the response
                $response = $email . ' ' . $this->hmac($challenge, $password);

                // send encoded credentials
                return $this->sendCommand('email', base64_encode($response), 235);
                break;
        }
        return true;
    }
